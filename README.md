...
В аргумент подается файл ".txt" с грамматикой, строка для для разбора, типы вывода (silent -- просто получить результат, show_steps -- пошагово заполнить таблицу алгоритма CYK) и файл для вывода туда итогового разбора, если файла нет, то он создастся.

Пример запуска:

``./run.sh tests/1.txt "'a''b''a''a''b''a'" show_steps answer.txt``
